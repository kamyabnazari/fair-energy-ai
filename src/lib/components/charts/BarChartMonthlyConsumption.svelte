<script>
	...
	export let monthlyConsumptionData = []; // Accept data as a prop
	...
	let sortedData = monthlyConsumptionData;
	$: reactiveShowSort = (input) => {
		if (input === 1) {
			return (sortedData = monthlyConsumptionData.sort(
				(a, b) => a[x].charCodeAt(0) - b[x].charCodeAt(0)
			));
		}
		if (input === 2) {
			return (sortedData = monthlyConsumptionData.sort((a, b) => a[y] - b[y]));
		}
		if (input === 3) {
			return (sortedData = monthlyConsumptionData.sort((a, b) => b[y] - a[y]));
		}
	};

	// Compute values X and Y value of Arrays
	const x = Object.keys(monthlyConsumptionData[0])[0]; // given d in data, returns the (ordinal) x-value
	const y = Object.keys(monthlyConsumptionData[0])[1]; // given d in data, returns the (quantitative) y-value
