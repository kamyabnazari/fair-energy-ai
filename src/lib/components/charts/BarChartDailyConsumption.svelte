<script>
	...
	export let dailyConsumptionData = []; // Accept data as a prop
	...
	let sortedData = dailyConsumptionData;
	$: reactiveShowSort = (input) => {
		if (input === 1) {
			return (sortedData = dailyConsumptionData.sort(
				(a, b) => a[x].charCodeAt(0) - b[x].charCodeAt(0)
			));
		}
		if (input === 2) {
			return (sortedData = dailyConsumptionData.sort((a, b) => a[y] - b[y]));
		}
		if (input === 3) {
			return (sortedData = dailyConsumptionData.sort((a, b) => b[y] - a[y]));
		}
	};

	// Compute values X and Y value of Arrays
	const x = Object.keys(dailyConsumptionData[0])[0]; // given d in data, returns the (ordinal) x-value
	const y = Object.keys(dailyConsumptionData[0])[1]; // given d in data, returns the (quantitative) y-value
